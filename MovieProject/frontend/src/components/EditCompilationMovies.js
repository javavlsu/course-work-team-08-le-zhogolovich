...
const EditCompilationMovies = ({ compilationId, movies, onMovieRemoved }) => {
  ...
  const handleRemove = async (movieId) => {
    if (
      !window.confirm("Вы уверены, что хотите удалить этот фильм из подборки?")
    )
      return;
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await apiClient.delete(
        `/compilations/${compilationId}/movie/${movieToDelete.id}`,
      );
  ...
  if (!movies || movies.length === 0) {
    return (
      <p className="text-white-50 mt-4 text-center">
        ...
        {movies.map((movie) => (
          ...
          >
            <div className="d-flex align-items-center gap-3">
              <img
                src={
                  movie.coverUrl ? getImageUrl(movie.coverUrl) : avatarDefault
                }
                alt={movie.name}
                style={{
                  width: "45px",
