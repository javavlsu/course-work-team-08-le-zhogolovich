    ...
    <div className="container mt-5 pt-5">
      <div className="mx-auto" style={{ maxWidth: "600px" }}>
        ...
        {users.length > 0 ? (
          <div className="d-flex flex-column gap-3">
            {users.map((u) => (
              ...
              >
                <img
                  src={
                    u.avatarUrl ? `${getImageUrl(u.avatarUrl)}` : avatarDefault
                  }
                  alt={u.username}
                ...
                <div>
                  ...
                  <div className="text-white-50 small">
                    {u.aboutMe?.substring(0, 50) || "Киноман"}
                  </div>
                </div>
              </Link>
            ))}
          ...
          <div className="text-center text-white-50 mt-5">Здесь пока пусто</div>
        )}

        <div className="mt-4 text-center">
          <Link
            to={username === "me" ? "/profile" : `/users/${username}`}
            className="nav-btn text-white-50"
          >
            ← Вернуться в профиль
          </Link>
        </div>
